...
if exists("*searchpairpos")
    ...
    if !exists('g:hylang_maxlines')
        let g:hylang_maxlines = 100
    endif

    if !exists('g:hylang_fuzzy_indent')
        let g:hylang_fuzzy_indent = 1
    endif

    if !exists('g:hylang_fuzzy_indent_patterns')
        let g:hylang_fuzzy_indent_patterns = ['^with', '^def', '^let']
    endif

    if !exists('g:hylang_fuzzy_indent_blacklist')
        let g:hylang_fuzzy_indent_blacklist = ['-fn$', '\v^with-%(meta|out-str|loading-context)$']
    endif

    if !exists('g:hylang_special_indent_words')
        let g:hylang_special_indent_words = 'deftype,defrecord,reify,proxy,extend-type,extend-protocol,letfn'
    endif

    if !exists('g:hylang_align_multiline_strings')
        let g:hylang_align_multiline_strings = 0
    endif
    ...
    function! s:MatchPairs(open, close, stopat)
        ...
        " comments.
        if a:stopat == 0
            let stopat = max([line(".") - g:hylang_maxlines, 0])
        else
            let stopat = a:stopat
        endif
    ...
    function! s:HylangCheckForStringWorker()
    ...
    function! s:CheckForString()
        ...
        try
            let val = s:HylangCheckForStringWorker()
        finally
    ...
    function! s:HylangIsMethodSpecialCaseWorker(position)
        ...
        call search('\S', 'W')
        if g:hylang_special_indent_words =~ '\<' . s:CurrentWord() . '\>'
            return 1
        endif
    ...
    function! s:IsMethodSpecialCase(position)
        let pos = s:SavePosition()
        try
            let val = s:HylangIsMethodSpecialCaseWorker(a:position)
        finally
            call s:RestorePosition(pos)
        endtry
        return val
    ...
    function! GetHylangIndent()
        ...
        if i > -1
            return i + !!g:hylang_align_multiline_strings
        endif
        ...
        "   g:hylang_special_indent_words. These are special cases.
        ...
        " metacharacters.
        "
        " e.g. hylang.core/defn and #'defn should both indent like defn.
        ...
        if g:hylang_fuzzy_indent
            \ && !s:MatchesOne(g:hylang_fuzzy_indent_blacklist, ww)
            \ && s:MatchesOne(g:hylang_fuzzy_indent_patterns, ww)
            return paren[1] + &shiftwidth - 1
        endif
    ...
    setlocal indentexpr=GetHylangIndent()
...
" Specially indented symbols from hylang.core and clojure.test.
"
" Hylang symbols are indented in the defn style when they:
"
...
setlocal lispwords+=deftest " hylang.test
setlocal lispwords+=deftest- " hylang.test
...
setlocal lispwords+=set-test " hylang.test
...
setlocal lispwords+=testing " hylang.test
...
setlocal lispwords+=with-test " hylang.test
